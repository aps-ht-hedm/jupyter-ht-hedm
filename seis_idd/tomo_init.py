...
def get_detector(
    ...
    if mode.lower() == 'debug':
        ...
        det.cam.nd_attributes_file.put(str(Path(_devices['area_detector']['cam_layout']).absolute()))
        # set attributes for HDF5 plugin
        det.hdf1.xml_file_name.put(str(Path(_devices['area_detector']['hdf_layout']).absolute()))
...
# --- add the overall control function to initialize all devices
# NOTE:
#    6-ID-D might not have the following two PVs, or most likely use something
#    equivalent, but different in implementation
